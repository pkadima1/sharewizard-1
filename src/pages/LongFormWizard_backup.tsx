...
const LongFormWizard = () => {
  ...
  const { currentStep: initialStep, completedSteps: initialCompleted } = loadSavedProgress();
  
  const [currentStep, setCurrentStep] = useState(initialStep);
  const [completedSteps, setCompletedSteps] = useState<number[]>(initialCompleted);
  const [skippedSteps, setSkippedSteps] = useState<number[]>([]);const [formData, setFormData] = useState({
    topic: '',
    audience: '',
    tone: '',
    industry: '',
  ...
  const handleGenerate = () => {
    ...
    // If we have media files, they are already uploaded to Firebase Storage
    // and URLs are stored in formData.mediaFiles
    const mediaUrls = formData.mediaFiles.map(file => ({
      url: file.url,
      placement: file.metadata.mediaPlacement,
      caption: file.metadata.mediaCaption
    }));
    
    console.log('Media URLs for generation:', mediaUrls);
    
    // Clear the draft since content is being generated
    clearDraft();
    
    // For now, we'll just show a success message and redirect
    alert('Content generation started! You will be notified when it completes.');
    navigate('/dashboard');
  };  // Render different content based on the current step
  const renderStepContent = () => {
    switch (currentStep) {      case 0: // Media & Topic Selection
        return (
  ...
  const navigateToStep = (stepIndex: number) => {
    ...
    if (stepIndex <= Math.max(...completedSteps, -1) + 1 || completedSteps.includes(stepIndex)) {
  ...
  return (    <div className="container mx-auto p-4 mt-20">
      ...
      <Card className="max-w-4xl mx-auto p-6">        {/* Enhanced Progress Bar */}
        <div className="mb-6">
          {/* Progress Overview */}
