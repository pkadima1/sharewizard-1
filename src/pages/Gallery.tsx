    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-indigo-950 text-white">
      ...
      <header className="sticky top-0 z-10 bg-slate-900/60 backdrop-blur-xl border-b border-white/5 shadow-2xl">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 pt-16 pb-8 text-center">
          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold bg-gradient-to-r from-white via-blue-100 to-indigo-200 bg-clip-text text-transparent mb-2 mt-6 pb-3">
            {t('header.title', 'Generated by EngagePerfect') as string}
          </h1>
          <p className="text-slate-200 text-sm sm:text-base max-w-3xl mx-auto leading-relaxed">
            {t('header.subtitle', 'A showcase of AI-powered content designed to inspire, engage, and grow your presence.') as string}
          </p>
        ...
        {error && (
          <div className="text-center py-16">
            <div className="bg-red-500/5 border border-red-500/20 rounded-2xl p-8 max-w-md mx-auto backdrop-blur-sm">
              <div className="text-red-300 text-lg font-medium">{typeof t('error', { defaultValue: error }) === 'string' ? t('error', { defaultValue: error }) : (error as string)}</div>
            </div>
          </div>
        )}
          ...
          <>
            {images.length === 0 ? (
              <div className="text-center text-slate-400 py-16">
                <div className="text-lg">{typeof t('empty', { defaultValue: 'No images found in the gallery.' }) === 'string' ? t('empty', { defaultValue: 'No images found in the gallery.' }) : 'No images found in the gallery.'}</div>
              </div>
            ) : (
              <div className="columns-1 sm:columns-2 lg:columns-3 gap-6">
                {images.map((url, idx) => (
