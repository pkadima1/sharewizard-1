    ...
    <div ref={ref} className="caption-preview-container">
      <div id="sharable-content" className={mediaType === 'text-only' ? 'p-6' : ''}>
        {mediaType !== 'text-only' && previewUrl && (
          <div className="relative">            {mediaType === 'image' ? (
              <div className="w-full relative">
                ...
                {captionOverlayMode === 'overlay' && caption && (
                  <div className="absolute bottom-0 left-0 right-0 bg-black bg-opacity-70 p-4 backdrop-blur-sm">
                    <p className="text-white text-lg font-semibold mb-2">{caption.title}</p>
                    <p className="text-white text-sm mb-2">{caption.caption}</p>
                    <p className="text-white text-sm italic mb-2">{caption.cta}</p>
        ...
        {caption && (captionOverlayMode === 'below' || mediaType === 'text-only') && (
          <div className={`space-y-3 p-6 ${mediaType !== 'text-only' && captionOverlayMode === 'below' ? 'bg-blue-950 text-white' : ''}`}>
            <h3 className="font-semibold text-xl">{caption.title}</h3>
            <p className="whitespace-pre-line">{caption.caption}</p>
            <p className="italic text-gray-300 mt-3">{caption.cta}</p>
